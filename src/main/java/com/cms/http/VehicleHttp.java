...
public class VehicleHttp {
	...
	GenericUrl url = new GenericUrl(this.stonkamHostname+"/GetDeviceList/100?SessionId="+sessionId+"&User="+stk_user);
	...
	private List<VehicleDto>getDevicesByIds(List<Long> deviceList, List<Map<String, Object>> deviceObjects) throws IOException {
		Gson gson = new Gson();
		long sessionId = this.stonkamService.refreshAccessToken();
		GenericUrl url = new GenericUrl(this.stonkamHostname+"/GetDevicesGps/100?CmsClientId=0&IsNeedPush=0&SessionId="+sessionId);
